...
namespace Bismillah.UI
{
    public partial class CreateBill : Form
    {
        ...
        public CreateBill(int staffId)
        {
            ...
            // Configure grid after initialization
            ConfigureDataGridView();
            dvgProductsinBill.DataSource = _billItems;
        ...
        private void ConfigureDataGridView()
        {
            if (dvgProductsinBill == null)
            {
                throw new InvalidOperationException("DataGridView control is not initialized");
            }
            ...
            // Add columns
            AddGridColumn("sr_no", "Sr.No", 60);
            AddGridColumn("name", "Product", 200);
            AddGridColumn("quantity", "Qty", 60);
            AddGridColumn("unit_price_display", "Unit Price", 100);
            AddGridColumn("total_display", "Total", 100);

            // Hide technical columns (these will be created when data is bound)
            if (dvgProductsinBill.Columns.Contains("product_id"))
                dvgProductsinBill.Columns["product_id"].Visible = false;
            if (dvgProductsinBill.Columns.Contains("unit_price"))
                dvgProductsinBill.Columns["unit_price"].Visible = false;
            if (dvgProductsinBill.Columns.Contains("total"))
                dvgProductsinBill.Columns["total"].Visible = false;
        }

        private void AddGridColumn(string dataProperty, string headerText, int width)
        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (DataRow row in _billItems.Rows)
                {
                    ...
                    if (!_billBL.ValidateProductExists(productId))
                    {
                        MessageBox.Show($"Product ID {productId} is invalid or does not exist");
                        return;
                    }
                }
